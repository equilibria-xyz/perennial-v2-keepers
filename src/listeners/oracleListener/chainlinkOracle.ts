import { Address, Hex, PrivateKeyAccount, PublicClient, WalletClient, getContract } from 'viem'
import tracer from '../../tracer.js'
import { MultiInvokerAddress, ChainlinkFactoryAddress, SupportedChain } from '../../constants/network.js'
...
import { ChainlinkFactoryImpl } from '../../constants/abi/ChainlinkFactoryImpl.abi.js'
...
export class ChainlinkOracleListener {
  ...
  public async run(account: PrivateKeyAccount, signer: WalletClient) {
    ...
    console.log(`Running Chainlink Oracle Handler. Block: ${blockNumber}`)
    tracer.dogstatsd.gauge('chainlinkOracle.blockNumber', Number(blockNumber), { chain: this.chain.id })

    const commitments = (
      await this.getCommitments()
    )
      .map((commitment) => {
        tracer.dogstatsd.gauge('chainlinkOracle.awaitingVersions', commitment.awaitingVersions, {
    ...
    try {
      ...
      tracer.dogstatsd.increment('chainlinkOracle.transaction.sent', 1, {
        chain: this.chain.id,
      })
      ...
      if (receipt.status === 'success')
        tracer.dogstatsd.increment('chainlinkOracle.transaction.success', 1, {
          chain: this.chain.id,
        })
      if (receipt.status === 'reverted')
        tracer.dogstatsd.increment('chainlinkOracle.transaction.reverted', 1, {
          chain: this.chain.id,
        })
    } catch (error) {
      if (error.response) {
        console.error(`ChainlinkOracle Got error: ${error.response.status}, ${error.response.data}`)
      } else if (error.request) {
        console.error(`ChainlinkOracle got error: ${error.request}`)
      } else {
        console.error(`Chainlink Oracle got error: Error ${error.message}`)
      }
    }
  }
    ...
    const commitmentPromises = this.oracleAddresses.map(async ({ oracle, id }) => {
      ...
      const vaas = await Promise.allSettled(
        versionsToCommit.map(async ({ version, index }, i) => {
          ...
          const chainlinkResponse = await fetchReportSingleFeed(
            process.env.CHAINLINK_BASE_URL!,
            process.env.CHAINLINK_ID!,
            process.env.CHAINLINK_SECRET!,
            underlyingId,
            Number(publishTime),
          )
          const vaa = chainlinkResponse.fullReport
